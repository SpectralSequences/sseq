...
pub fn run_test(config : &Config) -> Result<(), Box<dyn Error>> {
    ...
    for t in 1..max_degree {
        for s in 1 ..= t {
            let num_gens = res.read().inner.module(s as u32).number_of_gens_in_degree(t);
            for i in 0 .. num_gens {
                let mut vec = vec![0;num_gens];
                vec[i] = 1;
                res.write().add_product(s as u32, t, vec, &format!("({},{},{})",s,t,i));
