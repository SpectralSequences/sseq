...
impl<M : Module, F : ModuleHomomorphism<M, M>, CC : ChainComplex<M, F>> Resolution<M, F, CC> {
    ...
    /// Call our resolution X, and the chain complex to resolve C. This is a legitimate resolution
    /// if the map f: X &rarr; C induces an isomorphism on homology. This is the same as saying the
    /// cofiber is exact. The cofiber is given by the complex
    ///
    /// > X<sub>s</sub> &oplus; C<sub>s+1</sub> &rarr; X<sub>s-1</sub> &oplus; C<sub>s</sub> &rarr; X<sub>s-2</sub> &oplus; C<sub>s-1</sub> &rarr; ...
    ///
    ...
    /// ```
    ///
    /// Our method of producing X<sub>s, t</sub> and the chain maps are as follows. Suppose we have already
    /// built the chain map and differential for X<sub>s-1, t</sub> and X<sub>s, t-1</sub>. Since X<sub>s</sub> is a free
    /// module, the generators in degree < t gives us a bunch of elements in X<sub>s</sub>already, and we
    /// know exactly where they get mapped to. Let T be the Fp vector space generated by these
    /// elements. Then we already have a map
    ///
    /// > T &rarr; X<sub>s-1, t</sub> &oplus; C<sub>s, t</sub>
    ///
    /// and we know this hits the kernel of the map
    ///
    /// > D = X<sub>s-1, t</sub> &oplus; C<sub>s, t</sub> &rarr; X<sub>s-2, t</sub> &oplus; C<sub>s-1, t</sub>.
    ///
    /// What we need to do now is to add generators to X<sub>s, t</sub> to hit the entirity of this kernel.
    /// Note that we don't *have* to do this. Some of the elements in the kernel might be hit by
    /// C<sub>s+1, t</sub> and we don't have to hit them, but we opt to add generators to hit it anyway.
    ///
    /// If we do it this way, then we know the composite of the map
    ///
    /// > T &rarr; X<sub>s-1, t</sub> &oplus; C<sub>s, t</sub> &rarr; C<sub>s, t</sub>
    ///
    /// has to be surjective, since the image of C<sub>s, t</sub> under D is also in the image of X<sub>s-1,
    /// t</sub>. So our first step is to add generators to X<sub>s, t</sub> such that this composite is
    /// surjective.
    ///
    /// After adding these generators, we need to decide where to send them to. We know their
    /// values in the C<sub>s, t</sub> component, but we need to use a quasi-inverse to find the element in
    /// X<sub>s-1, t</sub> that hits the corresponding image of C<sub>s, t</sub>. This tells us the X<sub>s-1, t</sub>
    /// component.
    ///
    /// Finally, we need to add further generators to X<sub>s, t</sub> to hit all the elements in the
    /// kernel of
    ///
    /// >X<sub>s-1, t</sub> &rarr; X<sub>s-2, t</sub> &oplus; C<sub>s-1, t</sub>
    ///
    ...
    /// > T &rarr; X<sub>s-1, t</sub> &oplus;C<sub>s, t</sub>
    ///
    /// > X<sub>s, t</sub> &rarr; X<sub>s-1, t</sub> &oplus;C<sub>s, t</sub>
    ///
    /// agree.
    ///
    ...
    ///  * `old_kernel` - The kernel of the map X<sub>s-1, t</sub> &rarr; X<sub>s-2, t</sub> &oplus; C<sub>s-1, t</sub>, computed
