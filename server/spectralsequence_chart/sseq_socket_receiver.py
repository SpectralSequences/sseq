...
class SseqSocketReceiver(SocketReceiver):
    ...
    @transform_outbound_messages
    async def consume_chart(self, source_agent_id, cmd, **kwargs):
        await self.send_message_to_socket(cmd, **kwargs)

    @transform_outbound_messages
    async def consume_display(self, source_agent_id, cmd, **kwargs):
        await self.send_message_to_socket(cmd, **kwargs)
