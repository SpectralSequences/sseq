...
class Resolution:
    ...
    def resolve(self, n):
        t = threading.Thread(target=self.resolve_thread(n), daemon=True)
        t.start()
    ...
    def add_sseq_class_handler(self, ss):
        async def handler(self, x, y, idx): 
            await ss.add_class(x, y) 
        self.add_class_handler(handler)
     
    def add_sseq_structline_handler(self, ss):
        async def handler(self, 
            source_x, source_y, source_idx, 
            target_x, target_y, target_idx
        ):
            source_class = ss.get_class_by_idx(source_x, source_y, source_idx)
            target_class = ss.get_class_by_idx(target_x, target_y, target_idx)
            await ss.add_structline(source_class, target_class)
        self.add_structline_handler(handler)

    def set_sseq(self, ss):
        self.ss = ss
        self.add_sseq_class_handler(ss)
        self.add_sseq_structline_handler(ss)
    ...
    async def after_step(self, s, t):
        for idx in range(self.res.module(s).number_of_gens_in_degree(t)):
