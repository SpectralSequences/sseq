...
class Execution:
    ...
    async def send_message_a(self, subcmd, *, last_response, **kwargs):
        await self.executor.send_message_a(
            "execute", self.uuid, subcmd=subcmd, last_response=last_response, **kwargs
        )
    ...
    def execution_context(self):
        ...
        with redirect_stdout(WriteStream(self.send_stdout_write)), redirect_stderr(
            WriteStream(self.send_stderr_write)
        ):
            try:
    ...
    async def send_syntax_errors_a(self, errors):
        await self.send_message_a(
            "validate_syntax", last_response=True, valid=False, errors=errors
        )
    ...
    async def send_exception_a(self, e, file):
        await self.send_message_a(
            "exception",
            last_response=True,
            traceback=Traceback.format_exception(e, file),
        )
