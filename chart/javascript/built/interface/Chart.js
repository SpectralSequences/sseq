function groupByArray(xs, key) {
    return xs.reduce(function reducer(rv, x) {
...
export class ChartElement extends HTMLElement {
    ...
    getClassAtScreenCoordinates(x, y) {
        return this.classesToDraw.find((c) => this.disp.context.isPointInPath(c._path, x - c._canvas_x, y - c._canvas_y));
    }
    ...
    updateElements(disp) {
        ...
        let [classes, edges] = this.sseq.getElementsToDraw(this.pageRange, disp.xmin - 1, disp.xmax + 1, disp.ymin - 1, disp.ymax + 1);
        ...
        for (let c of classes) {
            c.setPosition(disp.xScale(c.x) + c.getXOffset(), disp.yScale(c.y) + c.getYOffset(), size);
        }
    }
