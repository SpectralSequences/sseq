...
class SseqChart:
    ...
    def add_differential(
        self,
        page: int,
        source_arg: ChartClassArg,
        target_arg: ChartClassArg,
        auto: bool = True,
    ) -> ChartDifferential:
        """Add a differential.
    ...
    def add_structline(
        self, source_arg: ChartClassArg, target_arg: ChartClassArg
    ) -> ChartStructline:
    ...
    def _add_delete_message(self, target_object: Any):
        self._add_batched_message(
            target_object.uuid
            + "--delete",  # Don't merge delete event with earlier update
            dict(
                chart_id=self.uuid,
                target_type=type(target_object).__name__,
                command="delete",
    ...
    def _normalize_class_argument(self, class_arg: ChartClassArg) -> ChartClass:
        ...
        if not isinstance(class_arg, Iterable):
            raise TypeError(
                f'Class specifier argument must either be of type "ChartClass" or an iterable of integers.'
            )
        ...
        if not self.num_gradings <= len(class_arg2) <= self.num_gradings + 1:
            raise TypeError(
                f'Class specifier argument argument must have length "num_gradings" = {self.num_gradings} or "num_gradings" + 1 = {self.num_gradings+1}'
            )
    ...
    def get_class(self, *args: int) -> ChartClass:
        ...
        if not self.num_gradings <= len(args) <= self.num_gradings + 1:
            raise TypeError(
                f'Iterable class specifier argument argument must have length "num_gradings" = {self.num_gradings} or "num_gradings" + 1 = {self.num_gradings+1}'
            )
    ...
    def classes_in_degree(self, *args: int) -> list[ChartClass]:
        ...
        if len(args) != self.num_gradings:
            raise TypeError(
                f'Argument to "classes_in_degree" must have length "num_gradings" = {self.num_gradings}'
            )
    ...
    def register_class_style(self, class_style: ChartClassStyle):
        ...
        if not class_style.group_name:
            raise ValueError(
                "register_class_style called on class_style with empty group_name."
            )
        if class_style.group_name in self._class_styles:
            if (
                class_style.to_json()
                == self._class_styles[class_style.group_name].to_json()
            ):
                return
            raise ValueError(
                f'A different class_style with group_name "{class_style.group_name}" is already registered.'
            )
    ...
    def register_edge_style(self, edge_style: ChartEdgeStyle):
        ...
        if not edge_style.action:
            raise ValueError(
                "register_class_style called on class_style with empty action."
            )
        if edge_style.action in self._edge_styles:
            if edge_style.to_json() == self._edge_styles[edge_style.action].to_json():
                return
            raise ValueError(
                f'A different class_style with action "{edge_style.action}" is already registered.'
            )
        from copy import deepcopy
