...
export class PopupElement extends LitElement {
    ...
    firstUpdated(changedProperties) {
        ...
        let onContentResized = async function onContentResized(_entries) {
            if (!this.minimized) {
                let body_and_footer =
                    this.shadowRoot.querySelector('.body-footer');
                let body_and_footer_inner =
                    this.shadowRoot.querySelector('.body-footer-inner');
                await sleep(100);
    ...
    updated(changedProperties) {
        ...
        if (this.height) {
            ...
            body.style.height = `calc(${test_height} - ${
    ...
    render() {
        return html`
            <div
                class="content"
                style="margin-top:${this.top}px; margin-left:${this.left}px;"
            >
                <div class="header">
                    ...
                    ${!this.modal
                        ...
                        : ''}
                    <sseq-button class="close-btn" @click=${this.cancel}
                        >×</sseq-button
                    >
                </div>
                <div class="body-footer">
                    <div
                        class="body-footer-inner"
    ...
    move(e) {
        ...
        let pageX = Math.max(
            Math.min(e.pageX, boundingRect.width),
            boundingRect.left,
        );
        let pageY = Math.max(
            Math.min(e.pageY, boundingRect.height),
            boundingRect.top,
        );
    ...
    handleBodyClick() {
        ...
        if (
            ...
            !document.activeElement.closest('sseq-popup')
        ) {
            this.focus();
        }
    }
    ...
    focus() {
        ...
        if (focusElt) {
            ...
            while (focusElt.shadowRoot && found) {
                found = false;
                for (let elt of focusElt.shadowRoot.querySelectorAll(
                    '[focus]',
                )) {
    ...
    async minimize() {
        ...
        for (let btn of this.shadowRoot
            .querySelector('.body-footer')
            .querySelectorAll('sseq-button')) {
