...
export class ChartElement extends LitElement {
    ...
    start() {
        let uiElt = this.uiElt();
        uiElt.addEventListener('begin-reflow', () =>
            this.startContinuousRedraw(),
        );
    ...
    handleMouseDown(event: MouseEvent) {
        ...
        if (this.mouseover_class) {
            this.dispatchCustomEvent('click-class', {
                cls: this.mouseover_class,
            });
        }
    }
    ...
    updateMouseoverClass(coord: [number, number]) {
        ...
        if (
            idx === undefined ||
            this.glyph_instance_index_to_class[idx] === undefined
        ) {
        ...
        if (this.mouseover_class) {
            this.dispatchCustomEvent('mouseout-class', {
                cls: this.mouseover_class,
            });
        }
        this.mouseover_class = new_mouseover_class;
        if (new_mouseover_class) {
            this.dispatchCustomEvent('mouseover-class', {
                cls: new_mouseover_class,
            });
        }
    }
    ...
    changePage(delta: number) {
        ...
        let new_idx = Math.min(
            Math.max(this.page_idx + delta, min_idx),
            max_idx,
        );
    ...
    setPage(idx?: number) {
        ...
        this.updateChart();
        this.dispatchCustomEvent('page-change', {
            page: this.page,
    ...
    fixPageRangeAndIndex() {
        ...
        let idx = this.sseq.page_list
            .map(e => JSON.stringify(e))
            .indexOf(JSON.stringify(this.pageRange));
        this.pageRange = this.sseq.page_list[this.page_idx];
        this.page = this.pageRange[0];
    }
    ...
    setInitialRange() {
        if (this.sseq) {
            this._canvas!.set_current_xrange(
                this.sseq.initial_x_range[0],
                this.sseq.initial_x_range[1],
            );
            this._canvas!.set_current_yrange(
                this.sseq.initial_y_range[0],
                this.sseq.initial_y_range[1],
            );
        } else {
    ...
    getClassPosition(c: ChartClass): [number, number] {
        ...
        let { x, y } = (<any>this._canvas!)
            .glyph_position(position, offset)
            .toJSON();
        return [x, y];
    }
    ...
    updateChart() {
        ...
        for (let c of this.sseq.classes.values()) {
            ...
            let position = new Vec2(c.x!, c.y!);
            let offset = new Vec2(
                c.getXOffset(this.page),
                c.getYOffset(this.page),
            );
            let border_width = c.border_width[this.page];
            let glyph = this.getShapeGlyph(
                c.shape[this.page],
                tolerance,
                border_width,
            );
        ...
        for (let e of this.sseq.edges.values()) {
            ...
            let { start_tip, end_tip, bend, color, dash_pattern, line_width } =
                e.getEdgeStyle(this.page);
    ...
    _updateClassScale() {
        ...
        let [ymin, ymax] = this.current_yrange();
        let xstep =
            (this.clientWidth - this.leftMargin - this.rightMargin) /
            (xmax - xmin);
        let ystep =
            (this.clientHeight - this.topMargin - this.bottomMargin) /
            (ymax - ymin);
        let min_step = Math.min(xstep, ystep);
        this._glyph_scale = Math.max(
            Math.min(min_step / 100, max_glyph_scale),
            min_glyph_scale,
        );
