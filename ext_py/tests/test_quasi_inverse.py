...
def make_identity_qi():
    ...
    preimage = fp.Matrix.from_py(3, [[1, 0, 0], [0, 1, 0], [0, 0, 1]])
...
def test_apply_wrong_type_is_valueerror_not_runtimeerror():
    ...
    with pytest.raises(ValueError):
        qi.apply(123, 1, good)
    with pytest.raises(ValueError):
        qi.apply(fp.Matrix.from_py(3, [[1, 0, 0]]), 1, good)
...
def test_compute_quasi_inverse_from_matrix():
    ...
    m = fp.AugmentedMatrix2.from_py(3, rows)
    m.row_reduce()
    qi = m.compute_quasi_inverse()
    assert qi.prime == 3
    assert qi.source_dimension == 3
    assert qi.preimage.to_py() == [[0, 1, 0], [0, 2, 2]]
