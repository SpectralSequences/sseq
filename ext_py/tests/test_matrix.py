...
def test_matrix_construction_and_queries():
    ...
    assert m.is_zero
    assert len(m) == 2
    assert m.to_py() == [[0, 0, 0], [0, 0, 0]]
    assert repr(m).startswith("Matrix(7, ")


@pytest.mark.xfail(reason="Needs fix")
def test_matrix_to_py_zero_dimensions():
    # Regression: a matrix with zero columns and nonzero rows shouldn't panic
    assert fp.Matrix(2, 1, 0).to_py() == [[]]
    assert fp.Matrix(2, 3, 0).to_py() == [[], [], []]
    # Zero rows (with or without columns) is the empty list of rows.
    assert fp.Matrix(2, 0, 3).to_py() == []
    assert fp.Matrix(2, 0, 0).to_py() == []
...
def test_matrix_from_py_and_identity():
    m = fp.Matrix.from_py(7, [[1, 3, 6], [0, 3, 4]])
    assert m.to_py() == [[1, 3, 6], [0, 3, 4]]
    assert not m.is_zero

    ident = fp.Matrix.identity(5, 3)
    assert ident.to_py() == [[1, 0, 0], [0, 1, 0], [0, 0, 1]]


def test_matrix_from_rows_and_from_row():
    r0 = fp.FpVector.from_py(5, [1, 2, 3])
    r1 = fp.FpVector.from_py(5, [4, 0, 1])
    m = fp.Matrix.from_rows(5, [r0, r1], 3)
    assert m.to_py() == [[1, 2, 3], [4, 0, 1]]

    single = fp.Matrix.from_row(5, r0, 3)
    assert single.to_py() == [[1, 2, 3]]


def test_matrix_augmented_from_py():
    py_m = [[1, 3, 6], [0, 3, 4]]
    m = fp.AugmentedMatrix2.from_py(7, py_m)
    assert m.rows == 2
    assert m.columns == 12
    assert m.segment_starts == [0, 10]
    assert m.segment_ends == [3, 12]
    assert m.segment(0, 0).to_py() == py_m
    assert m.segment(1, 1).to_py() == [[1, 0], [0, 1]]
...
def test_invalid_prime_and_dims():
    with pytest.raises(ValueError):
        fp.Matrix(1, 2, 2)
    with pytest.raises(ValueError):
        fp.Matrix.from_py(4, [[1, 2]])
    with pytest.raises(ValueError):
        fp.Matrix.from_py(7, [[1, 2], [3]])


def test_row_access_and_getitem():
    m = fp.Matrix.from_py(5, [[1, 2, 3], [4, 0, 1]])
    row = m.row(1)
    assert row.prime == 5
    assert len(row) == 3
    assert row[0] == 4
...
def test_row_mut_reflects_in_parent():
    ...
    rm = m.row_mut(0)
    rm.set_entry(0, 9)
    assert m.to_py()[0] == [4, 2, 3]
    rm[1] = 3
    assert m.row(0)[1] == 3
    rm.scale(2)
    assert m.to_py()[0] == [3, 1, 1]
    rm.set_to_zero()
    assert m.to_py()[0] == [0, 0, 0]
    rm.add_basis_element(2, 1)
    assert m.to_py()[0] == [0, 0, 1]
...
def test_iter_mut_writes_through_and_counts_rows():
    ...
    for i, out in enumerate(rows):
        out.set_entry(i, 3)
    assert m.to_py() == [[3, 2, 3], [4, 3, 1]]
    # A zero-row matrix yields an empty iterator.
    assert list(fp.Matrix(5, 0, 3).iter_mut()) == []


def test_mutators():
    m = fp.Matrix.from_py(5, [[1, 2], [3, 4]])
    m.swap_rows(0, 1)
    assert m.to_py() == [[3, 4], [1, 2]]
    with pytest.raises(IndexError):
        m.swap_rows(0, 2)

    m.row_op(0, 1, 1)
    assert m.to_py() == [[4, 1], [1, 2]]
    with pytest.raises(ValueError):
        m.row_op(0, 0, 1)
...
def test_assign_requires_matching_shape():
    m = fp.Matrix.from_py(5, [[1, 2], [3, 4]])
    other = fp.Matrix.from_py(5, [[1, 1], [1, 1]])
    m.assign(other)
    assert m.to_py() == [[1, 1], [1, 1]]
...
def test_row_reduce_rank():
    m = fp.Matrix.from_py(2, [[1, 1, 0], [0, 1, 1], [1, 0, 1]])
...
def test_trim_and_rotate():
    ...
    m.trim(0, 2, 1)
    assert m.to_py() == [[2, 3], [0, 1]]

    n = fp.Matrix.from_py(5, [[1, 0], [2, 0], [3, 0]])
    n.rotate_down(0, 3, 1)
    assert n.to_py() == [[3, 0], [1, 0], [2, 0]]


def test_bytes_roundtrip():
    m = fp.Matrix.from_py(5, [[1, 2, 3], [4, 0, 2]])
    data = m.to_bytes()
    n = fp.Matrix.from_bytes(5, 2, 3, data)
    assert n.to_py() == m.to_py()
...
def test_row_len_revalidates_after_column_shrink():
    ...
    m = fp.Matrix.from_py(5, [[1, 2, 3, 4], [5, 6, 7, 8]])
    row = m.row(0)
    rm = m.row_mut(1)
    assert len(row) == 4
    assert len(rm) == 4
...
def test_compute_kernel_dimensions_and_membership():
    m = fp.AugmentedMatrix2.from_py(3, _KI_ROWS)
    m.row_reduce()
    ker = m.compute_kernel()
    assert ker.prime == 3
...
@pytest.mark.xfail(reason="Raises PanicException")
def test_compute_kernel_requires_row_reduce():
    m = fp.AugmentedMatrix2.from_py(3, [[1, 0, 1], [0, 1, 1]])
    with pytest.raises(ValueError):
        m.compute_kernel()
