...
def make_subspace(p, rows):
    ...
    for row in rows:
        s.add_vector(fp.FpVector.from_py(p, row))
    return s


def test_construction_and_queries():
    linear = make_subspace(2, [[0, 1, 0], [0, 0, 1]])
    offset = fp.FpVector.from_py(2, [1, 0, 0])
...
def test_offset_is_reduced():
    ...
    linear = make_subspace(2, [[0, 1, 0], [0, 0, 1]])
    offset = fp.FpVector.from_py(2, [1, 1, 0])
    aff = fp.AffineSubspace(offset, linear)
...
def test_contains_membership():
    ...
    assert aff.contains(fp.FpVector.from_py(2, [1, 1, 0]))
    assert fp.FpVector.from_py(2, [1, 1, 0]) in aff

    # [0,1,0] has the wrong first coordinate, so it is not contained.
    assert not aff.contains(fp.FpVector.from_py(2, [0, 1, 0]))
    assert fp.FpVector.from_py(2, [0, 1, 0]) not in aff
...
def test_sum_semantics():
    ...
    assert s.contains(fp.FpVector.from_py(2, [1, 1, 1]))
    assert not s.contains(fp.FpVector.from_py(2, [0, 0, 0]))
...
def test_invalid_construction_raises():
    ...
    with pytest.raises(ValueError):
        fp.AffineSubspace(fp.FpVector.from_py(2, [1, 0]), linear)

    # Offset prime must match the linear part's prime.
    with pytest.raises(ValueError):
        fp.AffineSubspace(fp.FpVector.from_py(3, [1, 0, 0]), linear)
