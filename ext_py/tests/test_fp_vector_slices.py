...
def test_fp_vector_iter_nonzero():
    ...
    assert v.iter_nonzero() == v.const[0:len(v)].iter_nonzero()
...
def test_fp_slice_mut_add_assign_and_masked_methods():
    ...
    source = source_v.const[0:3]

    target.add(source, 2)
    assert [v[i] for i in range(len(v))] == [1, 2, 4, 1, 0, 0]

    target.assign(fp.FpVector.from_py(5, [4, 0, 1]).const[0:3])
...
def test_fp_slice_mut_add_tensor():
    ...
    v.mut.add_tensor(
        1,
        2,
        fp.FpVector.from_py(5, [1, 2]).const,
        fp.FpVector.from_py(5, [3, 4]).const,
    )
...
def test_fp_slice_mut_add_tensor_distinct_and_aliased():
    ...
    vt.mut.add_tensor(1, 2, left.const, right.const)
    ...
    vt2.mut.add_tensor(1, 2, w.const, w.const)
    ...
    v.mut.add_tensor(1, 2, v.const[0:2], r.const[0:2])
...
def test_fp_slice_mut_accepts_fp_vector_operand():
    ...
    left = fp.FpVector.from_py(5, [1, 2])
    right = fp.FpVector.from_py(5, [3, 4])
    out.mut.add_tensor(1, 2, left, right)
    assert [out[i] for i in range(6)] == [0, 1, 3, 2, 1, 0]

    # A matrix row_mut accepting an FpVector operand
    m = fp.Matrix.from_py(5, [[1, 1, 1], [2, 2, 2]])
...
def test_fp_slice_mut_new_method_errors_are_python_exceptions():
    ...
    target = target_v.mut
    same_prime_short = fp.FpVector(5, 2).const
    other_prime = fp.FpVector(7, 3).const
