...
def test_add_gen_quotient_reduce_and_gens():
    ...
    sq = fp.Subquotient(3, 5)
    sq.quotient(fp.FpVector.from_py(3, [1, 1, 0, 0, 1]))
    sq.quotient(fp.FpVector.from_py(3, [0, 2, 0, 0, 1]))
    sq.add_gen(fp.FpVector.from_py(3, [1, 1, 0, 0, 0]))
    sq.add_gen(fp.FpVector.from_py(3, [0, 1, 0, 0, 0]))
    ...
    # vector in place.
    elt = fp.FpVector.from_py(3, [2, 0, 0, 0, 0])
...
def test_from_parts():
    sub = fp.Subspace(2, 3)
    sub.add_vector(fp.FpVector.from_py(2, [1, 0, 0]))
    sub.add_vector(fp.FpVector.from_py(2, [0, 1, 0]))
    quot = fp.Subspace(2, 3)
    quot.add_vector(fp.FpVector.from_py(2, [1, 0, 0]))
...
def test_invalid_vector_inputs_raise():
    ...
    with pytest.raises(ValueError):
        sq.quotient(fp.FpVector.from_py(5, [1, 0, 0]))
    with pytest.raises(ValueError):
        sq.add_gen(fp.FpVector.from_py(3, [1, 0]))
    with pytest.raises(ValueError):
        sq.reduce(fp.FpVector.from_py(3, [1, 0]))


def test_reduce_by_quotient():
    sq = fp.Subquotient(3, 3)
    sq.quotient(fp.FpVector.from_py(3, [1, 0, 0]))
    v = fp.FpVector.from_py(3, [1, 1, 0])
