...
def test_subspace_from_matrix():
    m = fp.Matrix.from_py(3, [[1, 0, 0], [0, 1, 2]])
...
def test_add_vector_and_contains():
    ...
    w = fp.FpVector.from_py(3, [0, 1, 0])
    assert not s.contains(w)
    assert w not in s

    # A scalar multiple is still contained.
    twice = fp.FpVector.from_py(3, [2, 0, 0])
    assert s.contains(twice)


def test_contains_space_and_sum():
    a = fp.Subspace(3, 3)
    a.add_vector(fp.FpVector.from_py(3, [1, 0, 0]))
    b = fp.Subspace(3, 3)
    b.add_vector(fp.FpVector.from_py(3, [0, 1, 0]))
    ...
    assert s.contains(fp.FpVector.from_py(3, [1, 0, 0]))
    assert s.contains(fp.FpVector.from_py(3, [0, 1, 0]))
    assert s.dimension <= s.ambient_dimension

    # Overlapping subspaces: the sum's dimension is the union's rank.
    c = fp.Subspace(3, 3)
    c.add_vector(fp.FpVector.from_py(3, [1, 0, 0]))
    overlap = a.sum(c)
...
def test_reduce_in_place():
    s = fp.Subspace(3, 3)
    s.add_vector(fp.FpVector.from_py(3, [1, 0, 0]))

    v = fp.FpVector.from_py(3, [2, 1, 0])
    s.reduce(v)
    assert v[0] == 0
    assert v[1] == 1
...
def test_bytes_roundtrip():
    ...
    assert restored.contains(fp.FpVector.from_py(3, [1, 0, 0]))
...
def test_invalid_inputs_raise():
    ...
    with pytest.raises(ValueError):
        s.contains(fp.FpVector.from_py(5, [1, 0, 0]))

    with pytest.raises(ValueError):
        s.add_vector(fp.FpVector.from_py(3, [1, 0]))
