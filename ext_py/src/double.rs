//! Port of the inline `mod double` from `ext/examples/sq0.rs`.
//!
//! This presents the "doubled" chain complex of a resolution: a chain complex
//! whose modules halve Steenrod operations degree-wise. It is used by the
//! `sq0` example to compute the action of `Sq^0` on `Ext`.
//!
//! Ported verbatim from `ext/examples/sq0.rs` (submodules `double_algebra`,
//! `double_module`, `double_chain_complex`), adjusting only crate paths to the
//! `ext_py` dependency names. Nothing here is exposed to Python directly; the
//! `DoubleChainComplex` pyclass in `lib.rs` wraps `DoubleChainComplex` over the
