...
python_utils::py_repr!(FpVector, inner, "FreedVector", {
    Ok(format!(
        "F{}Vector {}",
        inner.prime(),
        inner
...
impl FpVector {
    ...
    pub fn handle_index(&self, index : isize) -> PyResult<usize> {
        python_utils::handle_index(self.inner_unchkd().len(), index, "dimension", "vector")
    }

    pub fn check_index(&self, index : isize) -> PyResult<()> {
        python_utils::check_index(self.inner_unchkd().len(), index, "dimension", "vector")
    }
    ...
    pub fn check_lengths_match(&self, other : &FpVector, message : &str) -> PyResult<()> {
        if self.inner_unchkd().len() != other.inner_unchkd().len() {
            return Err(python_utils::exception!(ValueError,
                format!("{}", message)
            ))
        } else {
...
impl FpVector {
    ...
    pub fn assign(&mut self, other : &FpVector) -> PyResult<()> {
        ...
        self.check_lengths_match(other, "Cannot assign vectors when dimensions do not match.")?;
        self.inner_mut()?.assign(other.inner()?);
        Ok(())
    }
...
impl FpVector {
    ...
    pub fn from_list(p : u32, l : PyObject) -> PyResult<Self> {
        ...
        Ok(FpVector::box_and_wrap(FpVectorRust::from_slice(new_valid_prime(p)?, &vec)))
    }
    ...
    pub fn add(&mut self, other : &FpVector, c : i32) -> PyResult<()> {
        ...
        self.check_primes_match(other, "")?;
        self.check_lengths_match(other, "Cannot add vectors when lengths do not match.")?;
        let c =  self.reduce_coefficient(c);
    ...
    #[staticmethod]
    pub fn num_limbs(p : u32, dimension : usize) -> PyResult<usize> {
        Ok(FpVectorRust::num_limbs(new_valid_prime(p)?, dimension))
    }

    #[staticmethod]
    pub fn padded_len(p : u32, dimension : usize) -> PyResult<usize> {
        Ok(FpVectorRust::num_limbs(new_valid_prime(p)?, dimension))
    }
