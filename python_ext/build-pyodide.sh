#!/usr/bin/env bash
#
# Build the `sseq_ext` Python extension as a WebAssembly wheel for Pyodide.
#
...
# pinned below.
#
# The build differs from the native one in two ways:
#   * the `concurrent` (rayon/threads) feature is dropped — default Pyodide has
#     no threads;
#   * the dev-only `test-hooks` feature is dropped.
#
# Usage:
#   ./build-pyodide.sh
#
# Output:
#   dist/sseq_ext-*-wasm32.whl   (install in Pyodide via micropip)
#
set -euo pipefail
...
# --- 3. Build the wheel ------------------------------------------------------
# Pass-through args disable the native-only cargo features for the wasm build.
export MATURIN_PEP517_ARGS="--no-default-features --features pyo3/extension-module"
