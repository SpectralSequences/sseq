...
message_handlers.calculate_adem = function calculate_adem(m){
    if(!(m.prime in self.calculators)){
        self.calculators[m.prime] = self.wasm.WasmSteenrodCalculator.new(m.prime);
        self.calculators[m.prime].compute_basis(20);
    }
    let result = self.calculators[m.prime].evaluate_adem(m.input);
