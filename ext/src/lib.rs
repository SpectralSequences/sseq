...
//! # Examples
//!
//! The library comes with a number of "example" binaries that use the library for various purposes.
//! These can be used directly to perform specific computations, or act as actual examples for how
//! to use the library.
//!
//! ## Running an example
//!
...
//!
//!  - `--release` compiles the code in release mode. This increases compilation time, but results
//!    in a much faster binary. This also disables some expensive run-time sanity checks.
...
//!
//! These are supplied right after `cargo run`, in any order. In general, one should set all of
//! these flags for any non-trivial calculation at the prime 2. See the [Features](#features)
//! section for other features that can be enabled.
//!
//! Each example runs interactively, and prompts the user for input. For example, the following
//! session computes all filtration one products in $\Ext(C2, \F_2)$ and prints them to `stdout`:
//!
//! ```sh
...
//!
//! In general, we write the output to `stdout` and the prompts to `stderr`. This allows the user to
//! redirect the output to another file or program.
//!
//! ## Prompts and arguments
//! Each prompt asks the user for an input, which is submitted by pressing the Enter/Return key. If
//! the input is invalid, an error message is produced and the user is queried for the same input
//! again. To exit the program early, one sends a `SIGTERM`, e.g. via `Ctrl-C`.
//!
//! Some prompts are optional or have default values. To select the `None` or default option, simply
//! supply an empty input.
//!
//! To facilitate batch processing, answers to the prompt can be supplied as command line arguments
//! instead; the nth command line argument is treated as the answer to the nth prompt. For example,
//! the previous interaction can be called by
//! ```sh
...
//! Each Steenrod module is defined in a `json` file, and a collection of such modules are available
//! in the `steenrod_modules/` subdirectory. New modules can be defined using the
//! [`define_module`](../define_module/index.html) example.
//!
//! Modules are specified using their file names, excluding the `.json` extension. Module files are
...
//!
//! For example, the module defined by `steenrod_modules/Ceta.json` can be specified with the name
//! `Ceta`. It is possible to apply a degree shift to the module without having to define a new one.
//! For example, to shift `Ceta` by one, we supply `Ceta[1]`.
//!
//! When resolving a module, we have to pick a basis of the Steenrod algebra, which is either the
//! Adem basis or the Milnor basis. The default choice is the Milnor basis. We can specify the basis
//! by appending `@basis_name`. For example, if we want to resolve `Ceta[1]` with the Adem basis, we
//! can specify it as `Ceta[1]@adem`.
//!
//! ### Ext elements
//! Each Ext group comes with a basis. The ith basis element of $\Ext^{s, n + s}$ is denoted `x_(n,
//! s, i)`. If we want to specify an element in a particular Ext group, we either write it as a
//! linear combination of the `x_(n, s, i)`, or written as a vector of the form e.g. `[0, 1, 0]`. In
//! the latter case, the bidegree is implicit.
//!
//! ### Save directory
...
//!
//! In general, the data for each bidegree (or each generator in some cases) is stored in a separate
//! file in an appropriate directory. This lets us only load the data we need when doing a
...
//!
//! For products, the subdirectory will be named after the name of the product. One must not reuse a
//! name for different products; the script may produce and write erroneous results silently in such
//! cases (though it practice it is likely to hit some error sooner or later).
//!
//! If the script is compiled with the `zstd` feature, then it supports reading from zstd compressed
//! save files, where each save file is individually compressed. The script will first look for the
//! uncompressed file. If it does not exist, it then looks for the file with the same name but with
//! a `.zst` extension.
//!
...
//! - `zstd`: Support reading zstd-compressed save files. Note that the save files are still written
//!   uncompressed. If compression is desired, output files need to be compressed manually.
