...
/// Same extraction as [`get_partial_matrix`] but the products are multiplied on the CPU
/// via [`MilnorAlgebra::multiply_basis_element_by_element_2`] (the kernel's reference
/// algorithm). Isolates the extraction from the GPU batch: if this matches the CPU
/// `get_partial_matrix` but [`get_partial_matrix`] does not, the fault is the GPU batch,
/// not the extraction.
pub fn get_partial_matrix_cpu2(hom: &NassauDifferential, degree: i32, inputs: &[usize]) -> Matrix {
    use fp::vector::FpVector;
    let (mut matrix, products) = extract(hom, degree, inputs);
    let algebra = hom.target().algebra();
    for prod in &products {
        let out_deg = prod.r_degree + prod.s_degree;
        let s_dim = algebra.dimension(prod.s_degree);
        let mut s = FpVector::new(hom.prime(), s_dim);
        for &ti in &prod.term_indices {
            s.set_entry(ti, 1);
        }
        let mut tmp = FpVector::new(hom.prime(), algebra.dimension(out_deg));
        ...
        let mut target_row = matrix.row_mut(prod.row);
        for (i, _) in tmp.iter_nonzero() {
            target_row.add_basis_element(prod.out_offset + i, 1);
        }
    }
    matrix
}
