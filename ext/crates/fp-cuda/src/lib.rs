...
static RREF_PTX_IMAGE: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/row_reduce.ptx"));
...
pub struct GpuContext {
    ...
    /// Held so the loaded modules outlive the [`CudaFunction`]s taken from them; never read.
    #[allow(dead_code)]
    module: Arc<CudaModule>,
    /// Ditto, for `row_reduce.ptx`.
    #[allow(dead_code)]
    rref_module: Arc<CudaModule>,
...
impl GpuContext {
    ...
    pub fn new(device_id: usize) -> anyhow::Result<Self> {
        ...
        let rref_module =
            ctx.load_module(Ptx::from_src(String::from_utf8(RREF_PTX_IMAGE.to_vec())?))?;
        let panel_factor = rref_module.load_function("panel_factor")?;
        let panel_factor_coop = rref_module.load_function("panel_factor_coop")?;
        let pf_find_swap = rref_module.load_function("pf_find_swap")?;
        let pf_step = rref_module.load_function("pf_step")?;
        let pf_xor = rref_module.load_function("pf_xor")?;
        let mark_live = rref_module.load_function("mark_live")?;
        let promote_pivots = rref_module.load_function("promote_pivots")?;
        let promote_coop = rref_module.load_function("promote_coop")?;
        let zero_pivot_l = rref_module.load_function("zero_pivot_l")?;
        let gather_rows = rref_module.load_function("gather_rows")?;
        let block_reduce_rref = rref_module.load_function("block_reduce_rref")?;
        let block_reduce_coop = rref_module.load_function("block_reduce_coop")?;
        let br_cond = rref_module.load_function("br_cond")?;
        let br_xor = rref_module.load_function("br_xor")?;
        let gather_cols = rref_module.load_function("gather_cols")?;
        let xor_into_perm = rref_module.load_function("xor_into_perm")?;
