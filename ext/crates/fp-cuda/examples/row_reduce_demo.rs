...
//! agreement with `row_reduce_cpu`, the CPU M4RI reduction this port must match.
//!
...
fn main() -> anyhow::Result<()> {
    ...
    for &(rows, cols, rank) in cases {
        ...
        let ok_cpu = e == cpu;
        let ok = ok_rank && ok_piv && ok_rref && ok_cpu;
        println!(
            "  rows={rows} cols={cols} rank={ref_rank}: {} (rref {} vs-cpu {} rank {} piv {})",
