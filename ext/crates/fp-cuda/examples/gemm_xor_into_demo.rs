//! The fused trailing-update epilogue `gemm_xor_into` must reproduce a blocked elimination's
//! trailing update — `M[:, off:] ^= L·U`, in place over a persistent device buffer — bit-for-bit.
//!
//! For each shape we build well-formed random `M` (m×N), `L` (m×k), `U` (k×t)
//! with `off + t == N` and `off` a limb boundary, compute the reference on the
//! host (whole-limb XOR of the CPU product `L·U` into M's trailing limbs), run
//! the device epilogue, and compare the downloaded buffer.
//!
