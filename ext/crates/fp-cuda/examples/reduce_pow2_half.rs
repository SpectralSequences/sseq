...
//! reduction vs the CPU M4RI reducer on rank-(n/2) square F₂ matrices of size
//! 2^n. Extends to 2^18; the CPU baseline is skipped once it would take too long
//! — M4RI is ~size-driven and explodes — leaving the device GPU-only at the top
//! end.
//!
//! Policy (override via env EXP_MIN/EXP_MAX, M4RI_MAX_EXP):
//!   device  : always
//!   M4RI    : exp ≤ M4RI_MAX_EXP  (default 16; ~48 min at 2^17, ~10 h at 2^18)
//!
//! Correctness oracle per size: device RREF == M4RI (when run), else — GPU-only
//! — the known rank n/2 plus idempotence
...
fn main() -> anyhow::Result<()> {
    ...
    println!("=== device vs CPU M4RI — half-rank square 2^n, n={exp_min}..={exp_max} ===");
    println!(
        "{:>7}  {:>10}  {:>10}   {:>9}  {:>8}  {:>9}  check",
        "n", "device(s)", "m4ri(s)", "vs-m4ri", "rank", "Tbop/s"
    );
    ...
    for exp in exp_min..=exp_max {
        ...
        // CPU baseline (skipped past its threshold).
