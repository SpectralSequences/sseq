...
pub trait Algebra: std::fmt::Display + Send + Sync {
    ...
    /// Computes the product `c * r * s` of the two basis elements, and *adds* the result to `result`.
    ///
    ...
    /// Computes the product `c * r * s` of two general elements, and *adds* the result to `result`.
    ///
    /// `result` is not required to be aligned.
    // Ideally, `Slice` below should be a type parameter eventually.
    fn multiply(&self, mut result: SliceMut, c: u32, r: impl Into<Elem<Self, Slice>>, s: impl Into<Elem<Self, Slice>>, excess: i32) {
        let p = self.prime();
        match (r.into(), s.into()) {
            (Elem::Basis(r), Elem::Basis(s)) => self.multiply_basis_elements(result, c, r, s, excess),
            (Elem::Vector(r), Elem::Basis(s)) => {
                for (i, v) in r.iter_nonzero() {
                    self.multiply_basis_elements(
                        result.copy(),
                        (c * v) % *p,
                        BasisElem::new(r.degree(), i),
                        s,
                        excess,
                    );
                }
            }
            (Elem::Basis(r), Elem::Vector(s)) => {
                for (i, v) in s.iter_nonzero() {
                    self.multiply_basis_elements(
                        result.copy(),
                        (c * v) % *p,
                        r
                        BasisElem::new(s.degree(), i),
                        excess,
                    );
                }
            }
            (Elem::Vector(r), Elem::Vector(s)) => {
                for (i, v) in r.iter_nonzero() {
                    for (j, w) in s.iter_nonzero() {
                        self.multiply_basis_elements(
                            result.copy(),
                            (c * v * w) % *p,
                            BasisElem::new(r.degree(), i),
                            BasisElem::new(s.degree(), j),
                            excess,
                        );
                    }
                }
    ...
    fn element_to_string(&self, x: impl Into<Elem<Self, Slice>>) -> String {
        match x.into() {
            Elem::Basis(b) => self.basis_element_to_string(b),
            Elem::Vector(v) => {
                let mut result = String::new();
                let mut zero = true;
                for (idx, value) in v.iter_nonzero() {
                    zero = false;
                    if value != 1 {
...
pub trait JsonAlgebra: Algebra {
    ...
    fn json_from_basis(&self, BasisElem<Self>) -> serde_json::Value;
}
...
pub trait GeneratedAlgebra: Algebra {
    ...
    fn generator_to_string(&self, BasisElem<Self>) -> String {
    ...
    fn string_to_generator<'a, 'b>(&'a self, input: &'b str)
        -> nom::IResult<&'b str, BasisElem<Self>>;
