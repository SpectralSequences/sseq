...
use crate::algebra::{
    ...
    combinatorics::{MAX_XI_TAU, xi_degrees},
};
...
pub fn multiply_batch_on_gpu(
    ...
    // Optional host-marshal vs device timing split (set MILNOR_GPU_TIMING).
    let timing = std::env::var("MILNOR_GPU_TIMING").is_ok();
    ...
    if timing {
        eprintln!(
            "  [gpu] {total_pairs} pairs: marshal {marshal_ms:.1}  upload {upload_ms:.1}  kernel \
             {kernel_ms:.1}  readback {read_ms:.1}  device {device_ms:.1}  (ms)"
        );
    }
    result
}
