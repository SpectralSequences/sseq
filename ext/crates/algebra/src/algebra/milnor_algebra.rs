    ...
    pub fn multiply_basis_element_by_element_2(
        ...
        loop {
            'outer: for term in &terms {
                ...
                // The sweep emits entries in increasing index order, so assemble the answer in a
                // plain `u64` and store it once instead of read-modify-writing through
                // `PPart::set`, the same way the `PPartMultiplier` kernel does.
                let mut bits: u64 = 0;
                let mut n = 0;
                ...
                if basis.len() < matrix.col_sums.len() {
                    ...
                    for &mask in &matrix.masks[basis.len()..] {
                        push_entry(&mut bits, &mut n, mask);
                    }
                } else {
                    for j in matrix.col_sums.len()..std::cmp::min(basis.len(), matrix.masks.len()) {
                        ...
                        push_entry(&mut bits, &mut n, b | matrix.masks[j]);
                    }
                    if basis.len() < matrix.masks.len() {
                        ...
                        for j in matrix.masks.len()..basis.len() {
                            push_entry(&mut bits, &mut n, basis.get(j));
                        }
                    }
                }

                // The packed form does not represent trailing zeros, so the trim the unpacked
                // p-part needed here happens for free.
                working.p_part = PPart::from_bits(bits);
...
/// Append `entry` as the next p-part entry of `bits`, which currently holds entries `0..n`.
///
/// The admissible sweep in [`MilnorAlgebra::multiply_basis_element_by_element_2`] produces entries
/// in increasing index order into a value that starts at zero, so a shift and an `or` suffice — no
/// read-modify-write through the checked [`PPart::set`]. A zero entry only advances `n`, which is
/// why the trailing-zero trim an unpacked p-part needed is unnecessary here: the packed form does
/// not represent trailing zeros in the first place.
///
/// The range check is a debug assertion, backed by the degree bound `compute_basis` enforces: the
/// product has degree `r_degree + s_degree`, so every entry fits its field.
#[inline]
fn push_entry(bits: &mut u64, n: &mut usize, entry: PPartEntry) {
    debug_assert!(
        entry <= PPart::max_entry(*n),
        "p-part entry {entry} does not fit in the field at index {n}"
    );
    *bits |= (entry as u64) << PPart::shift(*n);
    *n += 1;
}
