...
impl<A: AdemAlgebraT> PolynomialAlgebra for BCp<A> {
    ...
    fn polynomial_generators_in_degree(&self, degree: i32) -> usize {
        if is_two_times_power_of_p(
            *self.prime() as i32,
            self.algebra().adem_algebra().generic,
            degree,
        ) {
            1
        } else {
    ...
    fn exterior_generators_in_degree(&self, degree: i32) -> usize {
        if degree == 1 && self.algebra().adem_algebra().generic {
            1
        } else {
            0
        }
    ...
    fn repr_poly_generator(&self, degree: i32, _index: usize) -> (String, u32) {
        let q = if self.algebra().adem_algebra().generic {
            2
        } else {
            1
        };
...
impl<A: AdemAlgebraT> PolynomialAlgebraModule for BCp<A> {
    ...
    fn sq_polynomial_generator_to_monomial(
        ...
        if is_two_times_power_of_p(p, self.algebra().adem_algebra().generic, input_degree)
            && sq == input_degree
        {
            result.degree = p * input_degree;
            let int_idx = self.polynomial_monomials().gen_deg_idx_to_internal_idx(
                result.degree,
                self.frobenius_on_generator(input_degree, input_idx)
                    .unwrap(),
            );
