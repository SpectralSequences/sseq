...
use crate::algebra::{
    adem_algebra::AdemBasisElement, combinatorics::TruncatedPolynomialMonomialBasis, AdemAlgebra,
    AdemAlgebraT, Algebra, PolynomialAlgebra, PolynomialAlgebraMonomial,
...
impl<A: AdemAlgebraT> KFpn<A> {
    ...
    fn action_on_generator_helper(
        ...
        let output_dim = self
            .adem_algebra()
            .dimension(output_degree - self.n, self.n);
        ...
        if self.adem_algebra().generic && output_degree % 2 == 1 {
            for (idx, c) in temp_vec.iter_nonzero() {
                let gen_index = self
                    .exterior_monomials()
                    .gen_deg_idx_to_internal_idx(output_degree, idx);
...
impl<A: AdemAlgebraT> PolynomialAlgebra for KFpn<A> {
    ...
    fn polynomial_generators_in_degree(&self, degree: i32) -> usize {
        ...
        self.adem_algebra()
            .dimension_unstable(degree - self.n, self.n)
    }
    ...
    fn repr_poly_generator(&self, degree: i32, index: usize) -> (String, u32) {
        ...
        let (root_degree, root_index, pow) =
            self.inverse_frobenius_table[degree as usize / two_or_one][index];
        ...
        let var_str = if root_degree == 0 {
            ...
            format!(
                "{}(iota{})",
                self.adem_algebra()
                    .basis_element_to_string(root_degree, root_index),
                self.n
            )
        };
        (var_str, pow)
    ...
    fn repr_ext_generator(&self, degree: i32, index: usize) -> String {
        ...
        if degree == 0 {
            ...
            format!(
                "{}(iota{})",
                self.adem_algebra().basis_element_to_string(degree, index),
                self.n
            )
        }
    }
    ...
    fn frobenius_on_generator(&self, degree: i32, index: usize) -> Option<usize> {
        ...
        debug_assert!(
            p * degree <= Module::max_computed_degree(self),
            "degree : {}, max_computed_degree : {}",
            degree,
            Module::max_computed_degree(self)
        );
        let two_or_one = if self.adem_algebra().generic { 2 } else { 1 };
    ...
    fn compute_generating_set(&self, degree: i32) {
        ...
        for d in self.inverse_frobenius_table.len() as i32..=degree / two_or_one {
            let dim = self
                .adem_algebra()
                .dimension_unstable(d * two_or_one - self.n, self.n);
            let mut table = Vec::with_capacity(dim);
            for idx in 0..dim {
                let b = self
                    .adem_algebra()
                    .basis_element_from_index(d * two_or_one - self.n, idx);
                let mut frob_num = 0;
        ...
        for d in self.frobenius_table.len() as i32..=(degree) / two_p_or_p {
            let dim = self
                .adem_algebra()
                .dimension_unstable(two_or_one * d - self.n, self.n);
            let mut table = Vec::with_capacity(dim);
            for idx in 0..dim {
                let b = self
                    .adem_algebra()
                    .basis_element_from_index(two_or_one * d - self.n, idx);
...
impl<A: AdemAlgebraT> PolynomialAlgebraModule for KFpn<A> {
    ...
    fn action_table(&self) -> &OnceVec<Vec<Vec<FpVector>>> {
        // degree -> square -> monomial idx -> result vector
    ...
    fn sq_polynomial_generator_to_monomial(
        &self,
        _result: &mut PolynomialAlgebraMonomial,
        _sq: i32,
        _input_degree: i32,
        _input_idx: usize,
    ) {
