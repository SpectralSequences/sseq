...
impl<A: AdemAlgebraT> FreeUnstableModule<A> {
    ...
    //     self.lock()
    // }
    pub fn adem_algebra(&self) -> &AdemAlgebra {
    ...
    pub fn add_num_generators(&self, degree: i32, num_gens: usize, gen_names: Option<Vec<String>>) {
...
mod tests {
    ...
    //     let A = Arc::new(SteenrodAlgebra::from(AdemAlgebra::new(p, *p != 2, false, true)));
    //     A.compute_basis(10);
    //     let M = FreeUnstableModule::new(Arc::clone(&A), "".to_string(), 0);
...
// uint FreeUnstableModule_element_toJSONString(char *result, FreeUnstableModule *this, int degree, Vector *element);
