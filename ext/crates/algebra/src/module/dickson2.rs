...
use crate::algebra::{
    combinatorics::TruncatedPolynomialMonomialBasis, AdemAlgebra, AdemAlgebraT, Algebra,
        ...
        if let Some((a, b)) = self.quadratic_terms(sq + degree) {
            ...
            if a == b && a != 0 {
                let int_idx = self
                    ...
                    .gen_deg_idx_to_internal_idx((a + b) << frob, 0);
            ...
            if a != 0 {
                let int_idx = self
                    .polynomial_monomials()
                    .gen_deg_idx_to_internal_idx(a << frob, 0);
                result.poly.set_entry(int_idx, 1);
            }
            if b != 0 {
                let int_idx = self
                    .polynomial_monomials()
                    .gen_deg_idx_to_internal_idx(b << frob, 0);
                result.poly.set_entry(int_idx, 1);
            }
            return;
        }
