...
impl SteenrodCalculator {
    ...
    pub fn evaluate_adem_to_string(&self, input : &str) -> error::Result<String>{
        self.evaluate_adem(input).map(|(d, vect)| self.adem_algebra.element_to_string(d, vect.as_slice()))
    }

    pub fn evaluate_milnor_to_string(&self, input : &str) -> error::Result<String>{
        self.evaluate_milnor(input).map(|(d, vect)| self.milnor_algebra.element_to_string(d, vect.as_slice()))
    }
