...
directories = [str(f.relative_to("src/python")) for f in pathlib.Path("src/python").glob("**/*") if f.is_dir()]
imports = [f'import {file.stem}_{idx} from "./{file}";' for idx, file in enumerate(files)]
export = [ f"'{file.relative_to('python')}' : {file.stem}_{idx}" for idx, file in enumerate(files) ]
